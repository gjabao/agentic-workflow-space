...
class LinkedInJobScraper:
    ...
    def process_single_company(self, job: Dict) -> Dict:
        ...
        1. Find decision maker (LinkedIn search)
    ...
    def find_decision_maker(self, company_name: str) -> Dict:
        """
        Find Founder/CEO using Google Search (LinkedIn profiles).

        Args:
            company_name: Company name

        Returns:
            Dict: {full_name, first_name, last_name, title, linkedin_url, description}
        """
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    logger.debug(f"Could not find DM for {company_name}: {e}")
                    return {}
