#!/usr/bin/env python3
"""
Crunchbase Lead Scraper v4.0 - Email-First Approach (Following Google Maps Logic)

NEW WORKFLOW (v4.0):
1. Scrape companies from Crunchbase via Apify
2. Find ALL emails first (up to 20 per company) via AnyMailFinder Company API
3. Extract names from emails (firstname.lastname@ → "Firstname Lastname")
4. Search LinkedIn profiles by name + company using RapidAPI
5. Validate if decision-maker based on title keywords:
...
6. Output ONLY leads with valid emails

KEY DIFFERENCES from v2.x:
- ❌ OLD: Find decision-maker first → search email (low success rate)
- ✅ NEW: Find emails first → extract names → find LinkedIn → validate title

Based on: execution/scrape_google_maps.py (lines 688-763, 971-1050, 165-262)
Directive: directives/scrape_crunchbase_leads.md
"""
...
class AnyMailFinder:
    ...
    Copied from Google Maps scraper (lines 688-763)
    """
...
class RapidAPIGoogleSearch:
    ...
    Copied from Google Maps scraper (lines 48-163)
    """
    ...
    def search_by_name(self, full_name: str, company_name: str, location: str = None) -> Dict:
        ...
        for attempt_num, (query, num_results) in enumerate(search_attempts, 1):
            ...
            if data and data.get('results'):
                ...
                if found['full_name']:
                    # Note: Success message already logged in _extract_person_from_results
                    return found
...
class CrunchbaseScraper:
    """
    Crunchbase Lead Scraper v4.0 - Email-First Approach
    Following Google Maps scraper workflow
    """

    def __init__(self):
        """Initialize scraper with API credentials"""
        logger.info("⏳ Initializing CrunchbaseScraper v4.0 (Email-First)...")

        # Load API keys
        self.apify_api_key = self._load_secret("APIFY_API_KEY", required=True)
        self.crunchbase_cookies = self._load_secret("CRUNCHBASE_COOKIES", required=True)
        ...
        logger.info("✓ CrunchbaseScraper v4.0 initialized")
    ...
    def extract_contact_from_email(self, email: str) -> Tuple[str, bool, float]:
        ...
        Copied from Google Maps scraper (lines 971-1050)
    ...
    def _format_funding_stage(self, stage: Any) -> str:
        ...
        stage_lower = stage.lower().replace('_', ' ').replace('-', ' ')

        # Title case each word
        formatted = ' '.join(word.capitalize() for word in stage_lower.split())
    ...
    def _format_funding_amount(self, amount: Any) -> str:
        ...
        # Handle dict format from Crunchbase
        if isinstance(amount, dict):
            amount = amount.get('value_usd', 0)
    ...
    def _format_categories(self, categories: Any) -> str:
        ...
        # Handle list of dicts from Crunchbase
        if isinstance(categories, list):
            cat_names = []
            for cat in categories:
                if isinstance(cat, dict):
                    cat_names.append(cat.get('value', ''))
                else:
                    cat_names.append(str(cat))
            return ', '.join(filter(None, cat_names))
    ...
    def enrich_single_company(self, company: Dict) -> List[Dict]:
        ...
        # Step 2-5: Process each email (PARALLEL for 2-3x speed boost)
        decision_makers = []
        ...
        def process_single_email(email: str) -> Optional[Dict]:
            ...
            # Step 3: Search LinkedIn by name + company
            ...
            # Check full name from LinkedIn, not extracted name from email
            # This prevents skipping different people with same first name (e.g., multiple "Ben" at different companies)
            ...
            # Step 4: Validate if decision-maker
        ...
        # PARALLEL PROCESSING: Process 5 emails at a time (20 emails / 5 workers = 4 batches)
        # This gives 2-3x speed boost while respecting API rate limits
    ...
    def run(self, search_url: str, limit: int = 25) -> str:
        """
        Main workflow:
        1. Scrape companies from Crunchbase
        ...
        # Print header
        logger.info("=" * 70)
        logger.info("CRUNCHBASE LEAD SCRAPER v4.0 - EMAIL-FIRST APPROACH")
        logger.info("=" * 70)
        logger.info(f"Search URL: {search_url[:80]}...")
        ...
        logger.info("=" * 70)
        logger.info("CRUNCHBASE SCRAPE SUMMARY (v4.0)")
        logger.info("=" * 70)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Crunchbase Lead Scraper v4.0 - Email-First Approach"
    )
    parser.add_argument(
        "--search-url",
        required=True,
        help="Crunchbase search URL"
    )
    parser.add_argument(
        "--limit",
        type=int,
