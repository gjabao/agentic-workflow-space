#!/usr/bin/env python3
"""
Reed.co.uk Job Scraper & Decision Maker Outreach System
Scrapes jobs from Reed.co.uk, finds decision makers (Founder/CEO/CFO), gets emails, and generates outreach messages.

v2.0 Email-First Workflow - Based on Glassdoor/Indeed/LinkedIn scrapers
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('.tmp/reed_scraper.log'),
...
class ReedJobScraper:
    # Actor IDs
    JOB_SCRAPER_ACTOR_ID = "shahidirfan/Reed-Job-Scraper"
    ...
    def __init__(self):
        ...
        self.azure_deployment = os.getenv("AZURE_OPENAI_DEPLOYMENT", "gpt-4.1")
        ...
        logger.info("✓ ReedJobScraper initialized")
    ...
    def normalize_company_name(self, company_name: str) -> str:
        ...
        # Remove common legal suffixes (UK-focused)
        suffixes = [
            ', Inc.', ', Inc',
            ', LLC', ' LLC',
            ', Ltd.', ', Ltd', ' Ltd.',
    ...
    def detect_company_type(self, company_name: str, industry: str = "", description: str = "", website_desc: str = "") -> str:
        ...
        # Financial Services
        if any(word in text for word in ['bank', 'financial', 'finance', 'capital', 'investment', 'asset management', 'insurance', 'credit', 'payment', 'fintech']):
            return 'Financial Services'

        # Healthcare & Medical
        if any(word in text for word in ['hospital', 'health', 'medical', 'clinic', 'healthcare', 'pharmaceutical', 'pharma', 'biotech', 'nhs', 'patient']):
        ...
        if any(word in text for word in ['nonprofit', 'non-profit', 'charity', 'foundation', 'government', 'municipal', 'council', 'institute']):
    ...
    def start_scraping_job(self, keyword: str, location: str = "", max_jobs: int = 100, max_pages: int = 20) -> str:
        """Start the Apify scraper job and return the run ID."""
        logger.info(f"🔍 Starting Reed.co.uk scrape: '{keyword}' in '{location}' (max={max_jobs})...")
        ...
        try:
            ...
            run_id = run["id"]
            logger.info(f"✓ Reed scraper started. Run ID: {run_id}")
            return run_id
        except Exception as e:
            logger.error(f"❌ Failed to start scraper: {e}")
            return ""

    def stream_jobs(self, run_id: str, max_jobs: int = 100) -> Generator[Dict, None, None]:
        """Yield jobs from the Apify dataset as they become available with exponential backoff.

        shahidirfan/Reed-Job-Scraper output format:
        - url: job URL
        - jobId: Reed job ID
        - title: job title
        - company: company name
        - location: job location
        - salary: salary string (e.g., "£10,000 - £50,000")
        - datePosted: ISO date string
        - employmentType: e.g., "Permanent"
        ...
        while True:
            ...
            if items:
                for item in items:
                    ...
                    posted_date_raw = item.get('datePosted', '')
    ...
    def find_decision_maker_by_name(self, person_name: str, company_name: str) -> Dict:
        ...
        Example: "John Smith" at "Acme Ltd" → finds John's profile
        """
    ...
    def find_decision_maker(self, company_name: str) -> Dict:
        ...
        search_attempts = [
            ...
            f'site:linkedin.com/in/ ("founder" OR "co-founder" OR "ceo" OR "chief executive officer" OR "owner" OR "managing partner" OR "president" OR "managing director") "{company_name}"',

            # Attempt 3: Very broad search without site restriction (last resort)
            f'("{company_name}" AND (cfo OR "chief financial officer" OR ceo OR founder OR president OR "managing director") linkedin profile)'
        ]
    ...
    def find_company_website(self, company_name: str, keywords: str = "") -> Dict:
        ...
        Strategy 1: RapidAPI Google Search with UK keywords (3 query variations)
        Strategy 2: RapidAPI Google Search without keywords (3 query variations)
        Strategy 3: Domain guessing with UK TLDs (.co.uk, .com, .org.uk, .uk, .io, .net)
        ...
        if self.rapidapi_key and keywords:
            queries_with_keywords = [
                f'"{company_name}" {keywords} UK official website',
                f'{company_name} {keywords} UK website',
                f'{company_name} {keywords} UK home'
            ]

            for attempt, query in enumerate(queries_with_keywords, 1):
                logger.info(f"  → Strategy 1.{attempt}: Google + UK keywords")
        ...
        # ========== STRATEGY 3: Domain Guessing with UK TLDs ==========
        logger.info(f"  → Strategy 3: Domain guessing (UK TLDs)")
    ...
    def _try_google_search(self, query: str) -> Dict:
        ...
        for attempt in range(max_retries):
            try:
                ...
                # Filter out social media and job boards (UK-focused)
                for result in organic_results:
                    url = result.get('url', '')
                    if any(skip in url.lower() for skip in ['linkedin.com', 'facebook.com', 'twitter.com', 'reed.co.uk', 'indeed.com', 'glassdoor.com', 'totaljobs.com', 'cv-library.co.uk']):
                        continue
                    return {
                        'url': url,
    ...
    def _guess_domain(self, company_name: str, keywords: str = "") -> str:
        """
        Guess company domain from name using smart heuristics (UK-focused).

        Returns: domain string (e.g., "acme.co.uk") or empty string
        """
        ...
        # Remove common legal suffixes (UK-focused) - use endswith to avoid removing from middle
        legal_suffixes = [
            ' inc.', ' inc', ' llc', ' ltd.', ' ltd', ' corp.', ' corp',
            ' co.', ' co', ' l.p.', ' lp', ' llp', ' plc', ' limited',
            ' corporation', ' company', ' incorporated', ' group'
        ]
        ...
        # UK-focused TLD options (prioritize .co.uk for UK companies)
        tld_options = []

        # UK keywords → prioritize .co.uk
        uk_keywords = ['uk', 'london', 'manchester', 'birmingham', 'leeds', 'glasgow', 'edinburgh', 'bristol', 'liverpool', 'sheffield']
        if any(kw in keywords.lower() for kw in uk_keywords):
            tld_options = ['.co.uk', '.com', '.org.uk', '.uk', '.io', '.net']
        # Healthcare/Education → prioritize .org.uk / .nhs.uk
        elif any(kw in keywords.lower() for kw in ['hospital', 'health', 'nhs', 'university', 'college', 'school']):
            tld_options = ['.org.uk', '.nhs.uk', '.ac.uk', '.co.uk', '.com', '.org']
        # Government → .gov.uk
        elif any(kw in keywords.lower() for kw in ['council', 'government', 'municipal']):
            tld_options = ['.gov.uk', '.org.uk', '.co.uk', '.com']
        # Tech companies → try .io
        elif any(kw in keywords.lower() for kw in ['technology', 'software', 'tech', 'saas', 'ai', 'data']):
            tld_options = ['.com', '.io', '.co.uk', '.ai', '.co', '.net']
        # Default → UK-first comprehensive list
        else:
            tld_options = ['.co.uk', '.com', '.org.uk', '.uk', '.io', '.net', '.org']
    ...
    def _names_match(self, name1: str, name2: str, threshold: float = 0.6) -> bool:
        ...
        Examples:
        - "John Smith" vs "John Smith" → True (exact)
        ...
        - "John Smith" vs "Greg Chow" → False (different)
        """
    ...
    def is_decision_maker(self, job_title: str) -> bool:
        """Validate if job title is decision-maker (v3.0 broadened)"""
        if not job_title or len(job_title) < 3:
            return False
        jt_lower = job_title.lower()

        # Tier 1: C-suite / Founders (always pass, ignore excludes)
        tier1_keywords = ['founder', 'co-founder', 'ceo', 'chief', 'owner', 'president',
                         'cfo', 'cto', 'coo', 'cmo', 'c-suite', 'c-level']
        if any(kw in jt_lower for kw in tier1_keywords):
            return True

        # Tier 2: Senior leadership (VP, Director, Partner, Head)
        tier2_keywords = ['managing partner', 'managing director', 'vice president', 'vp ',
                         'svp', 'evp', 'executive director', 'director', 'principal',
                         'partner', 'head of', 'head ']
        if any(kw in jt_lower for kw in tier2_keywords):
            return True

        # Tier 3: Manager / Lead / Senior (pass unless truly junior)
        tier3_keywords = ['manager', 'lead', 'senior', 'executive']
        has_tier3 = any(kw in jt_lower for kw in tier3_keywords)

        # Only exclude truly junior titles
        exclude_keywords = ['intern', 'trainee', 'apprentice', 'student', 'clerk']
        has_exclude = any(kw in jt_lower for kw in exclude_keywords)

        return has_tier3 and not has_exclude
    ...
    def _google_search(self, query: str, num_results: int = 3) -> Optional[Dict]:
        """Generic Google search via RapidAPI (for signal checks)."""
        if not self.rapidapi_key:
            return None
        with self.rapidapi_lock:
            elapsed = time.time() - self.last_rapidapi_call
        ...
        if wait_time > 0:
            time.sleep(wait_time)
        try:
            headers = {'x-rapidapi-host': 'google-search116.p.rapidapi.com', 'x-rapidapi-key': self.rapidapi_key}
            params = {'query': query, 'num': str(num_results)}
            response = requests.get(self.RAPIDAPI_GOOGLE_SEARCH_URL, headers=headers, params=params, timeout=10)
            if response.status_code == 200:
                data = response.json()
                return {'results': [{'url': r.get('url', ''), 'title': r.get('title', ''), 'snippet': r.get('description', '')} for r in data.get('results', [])]}
        except Exception as e:
    ...
    def estimate_company_size(self, company_name: str) -> Tuple[str, int]:
        ...
        data = self._google_search(f'"{company_name}" site:linkedin.com/company/ employees', 3)
    ...
    def has_internal_recruiter(self, company_name: str) -> Tuple[bool, str]:
        ...
        recruiter_titles = ["recruiter", "talent acquisition", "people operations", "head of talent"]
        titles_query = ' OR '.join(f'"{t}"' for t in recruiter_titles)
        data = self._google_search(f'site:linkedin.com/in/ ({titles_query}) "{company_name}"', 3)
        if not data or not data.get('results'):
            return (False, "")
        for result in data['results']:
            url = result.get('url', '')
            ...
            if 'linkedin.com/in/' not in url:
                continue
            if any(kw in combined for kw in recruiter_titles) and company_name.lower() in combined:
                evidence = result.get('title', '')[:50]
    ...
    def extract_company_keywords(self, job_data: Dict) -> str:
        ...
        # Extract location (city/region) - UK-focused
        location = job_data.get('location', '')
        if location:
            # Clean location string
            loc_parts = location.replace(',', ' ').split()
            # Take first 2-3 parts (e.g., "London UK" or "Manchester England")
            keywords.extend(loc_parts[:3])

        # Add UK as keyword for UK job boards
        if 'UK' not in keywords and 'uk' not in [k.lower() for k in keywords]:
            keywords.append('UK')
        ...
        industry_keywords = {
            ...
            'healthcare': ['medical', 'health', 'clinical', 'hospital', 'doctor', 'nurse', 'pharmaceutical', 'nhs'],
            ...
            'legal': ['legal', 'attorney', 'counsel', 'law', 'compliance', 'solicitor', 'barrister'],
            'marketing': ['marketing', 'brand', 'digital', 'content', 'growth', 'seo'],
            'sales': ['sales', 'business development', 'account', 'revenue'],
            'data': ['data', 'analytics', 'insights', 'reporting', 'dashboard', 'bi '],
        }
...
7. Remove company legal suffixes (Inc., LLC, Ltd., etc.)
...
"""
    ...
    def process_single_company(self, job: Dict) -> List[Dict]:
        ...
        1. Find website (4-strategy validation with UK TLDs)
        ...
        # COMPREHENSIVE list of UK recruitment agencies and staffing firms (v3.0)
        recruitment_keywords = [
            # Generic keywords (high precision)
            'recruiting', 'recruitment', 'staffing', 'headhunter', 'talent acquisition',
            'search firm', 'resourcing', 'personnel', 'employment agency', 'job agency',
            'executive search', 'talent partner', 'talent solution', 'people solution',

            # Major UK/Global recruitment agencies (alphabetical)
            'adecco', 'ambition', 'antal', 'apple recruitment', 'aspire', 'austin fraser',
            'badenoch', 'blue arrow', 'brook street', 'capita', 'corestaff',
            'coburg banks', 'cornwall personnel', 'cv-library', 'cw jobs',
            'fawkes & reece', 'g2', 'gravitas', 'harnham',
            'harvey nash', 'hays', 'hudson', 'insight recruitment',
            'investigo', 'kelly services', 'korn ferry', 'la fosse',
            'lorien', 'macmillan davies', 'manpower', 'marks sattin',
            'matchtech', 'mercer', 'michael page', 'morgan hunt', 'morgan mckinley',
            'nigel frank', 'nonstop', 'office angels', 'opus',
            'page group', 'pagegroup', 'page personnel', 'pertemps', 'phaidon',
            'purple cv', 'randstad', 'reed', 'reed.co.uk', 'robert half', 'robert walters',
            'sellick', 'spencer stuart', 'spring technology', 'sthree',
            'talent works', 'the portfolio group', 'tiger', 'totaljobs',
            'understanding recruitment', 'vanrath', 'walker lovell', 'xpert',

            # IT/Tech-specific recruiters
            'computer futures', 'ecom', 'experis', 'hire digital', 'huxley',
            'ic resources', 'ignite', 'jefferson frank', 'leap29', 'modis',
            'net-recruit', 'oscar', 'proactive', 'talent international',

            # Finance/Accounting-specific recruiters
            'bdo', 'marks sattin', 'parkside', 'purely accountancy', 'rec2rec',

            # Sales/Marketing-specific recruiters
            'beresford blake', 'blu digital', 'ems', 'forward role',

            # Patterns to catch unknown agencies
            'agency', 'agencies', 'recruits', 'recruiter', 'recruiters',
            'hr solutions', 'workforce', 'temps', 'temporary staff',
            'consultants ltd', ' plc ', 'careers ', 'jobs ',
            'human resources', 'people business', 'people group'
        ]
        company_lower = company.lower()
        if company_lower == 'confidential' or any(kw in company_lower for kw in recruitment_keywords):
            logger.info(f"\n{'='*70}")
        ...
        if not getattr(self, '_skip_size_check', False):
            ...
            if size_bucket == "skip":
                ...
                self._skipped_too_large = getattr(self, '_skipped_too_large', 0) + 1
                return []
        else:
        ...
        if not getattr(self, '_skip_recruiter_check', False):
            ...
            if has_recruiter:
                ...
                self._skipped_has_recruiter = getattr(self, '_skipped_has_recruiter', 0) + 1
                return []
        else:
            has_recruiter = False
        ...
        def process_single_email(email: str) -> Optional[Dict]:
            ...
            if is_generic:
                # Keep generic emails as default DM contacts
                logger.info(f"    → Generic email - keeping as default DM")

                # Generate message for generic email
                msg = ""
                if self.openai_client:
                    msg = self.generate_message(
                        company, company, job['job_title'], '', company_desc
                    )
                ...
                return {
                    ...
                    'dm_email': email,
                    'email_status': 'found',
                    'dm_source': 'Generic company email',
                    'dm_description': '',
                    'message': msg
                }
            ...
            # Step 4: Validate name match (CRITICAL - prevents name mismatch bugs)
            ...
            # Step 6: Validate decision-maker title (size-aware)
            if size_bucket != "unknown":
                is_dm = self.is_decision_maker_by_size(job_title, size_bucket)
            else:
                is_dm = self.is_decision_maker(job_title)
            if not is_dm:
                logger.info(f"    ⚠ Not a decision-maker for {size_bucket} company: {full_name} ({job_title})")
            else:
            ...
            return {
                ...
                'message': msg,
                '_is_dm': is_dm  # Internal flag for DM vs fallback
            }

        # v3.0: Process at least 15 emails, aim for 3+ DMs, use fallbacks
        MIN_EMAILS_TO_TRY = 15
        MIN_DMS_TARGET = 3
        dm_results = []       # Confirmed decision-makers
        fallback_results = [] # Non-DM contacts (kept as backup)
        emails_tried = 0

        # Process ALL available emails (up to 20) in parallel
        with ThreadPoolExecutor(max_workers=5) as executor:
            ...
            for future in as_completed(futures):
                try:
                    ...
                    emails_tried += 1
                    if result:
                        if result.get('_is_dm', True):  # Generic emails default to True
                    ...
                    logger.error(f"  ❌ Error processing {email_addr}: {str(e)}")

        # Combine: DMs first, then fill with fallbacks if needed
        decision_makers = dm_results.copy()
        if len(decision_makers) < MIN_DMS_TARGET and fallback_results:
            needed = MIN_DMS_TARGET - len(decision_makers)
            logger.info(f"  → Only {len(dm_results)} DMs found, adding {min(needed, len(fallback_results))} fallback contacts")
            decision_makers.extend(fallback_results[:needed])

        # Clean internal flag before returning
        for dm in decision_makers:
            dm.pop('_is_dm', None)

        if emails_tried < MIN_EMAILS_TO_TRY and len(decision_makers) == 0:
            logger.info(f"  ⊘ Only tried {emails_tried}/{MIN_EMAILS_TO_TRY} emails, 0 DMs - insufficient data")
        else:
            logger.info(f"  ✓ Found {len(dm_results)} DMs + {len(fallback_results)} fallbacks ({emails_tried} emails tried) for {company}")
    ...
    def export_to_google_sheets(self, rows: List[Dict], title: str) -> str:
        ...
        try:
            ...
            headers = [
                "Company Name", "Company Type", "Company Website", "Est. Employees", "Company Size",
                "Job Title", "Job URL", "Location", "Job Age (Days)", "Pain Level",
    ...
    def execute(self, keyword: str, location: str = "", posted_date: str = "last14days", max_jobs: int = 10, max_pages: int = 20,
                min_age: int = 0, max_size: int = 300, skip_recruiter_check: bool = False, skip_size_check: bool = False):
        start_time = time.time()

        # Store config for signal checks
        self._max_company_size = max_size
        self._skip_recruiter_check = skip_recruiter_check
        self._skip_size_check = skip_size_check
        self._min_job_age = min_age
        self._skipped_too_large = 0
        self._skipped_has_recruiter = 0
        self._size_counts = {'small': 0, 'medium': 0, 'large': 0, 'unknown': 0}
        self._pain_counts = {'high': 0, 'medium': 0, 'low': 0, 'unknown': 0}

        print("\n" + "="*70)
        print("🚀 REED.CO.UK JOB SCRAPER & OUTREACH SYSTEM (STREAMING MODE)")
        print(f"Keyword: {keyword} | Location: {location} | Posted: {posted_date}")
        print(f"Signals: Size≤{max_size} | Recruiter={'OFF' if skip_recruiter_check else 'ON'} | MinAge={min_age}d")
        print("="*70 + "\n")

        # 1. Start Scraper
        run_id = self.start_scraping_job(keyword, location, max_jobs=max_jobs, max_pages=max_pages)
        if not run_id:
            return
        ...
        with ThreadPoolExecutor(max_workers=self.MAX_WORKERS) as executor:
            ...
            # Producer: Stream jobs from Apify (with max_jobs limit)
            for job in self.stream_jobs(run_id, max_jobs=max_jobs):
        ...
        num_companies = len(seen_companies)

        total_time = time.time() - start_time
        avg_time_per_company = total_time / num_companies if num_companies else 0
        success_rate = len(jobs_with_emails) / num_companies * 100 if num_companies else 0

        # Count signal stats from results
        for j in jobs_with_emails:
            sz = j.get('company_size_bucket', 'unknown')
            if sz in self._size_counts:
                self._size_counts[sz] += 1
            pl = j.get('pain_level', 'unknown')
            if pl in self._pain_counts:
                self._pain_counts[pl] += 1

        high_pain = self._pain_counts.get('high', 0)

        print("\n" + "-"*70)
        print(f"📊 PERFORMANCE METRICS:")
        print(f"   Companies Processed: {num_companies}")
        print(f"   Skipped (>{self._max_company_size} employees): {self._skipped_too_large}")
        print(f"   Skipped (Internal Recruiter): {self._skipped_has_recruiter}")
        print(f"   High Pain (30+ days): {high_pain}")
        print(f"   Size: Small(<50): {self._size_counts['small']}, Medium(50-150): {self._size_counts['medium']}, Large(150-300): {self._size_counts['large']}")
        if num_companies:
            print(f"   Decision Makers Found: {len(jobs_with_emails)} ({len(jobs_with_emails)/num_companies:.1f} per company)")
        ...
        logger.info(f"METRICS: companies={num_companies} dms={len(jobs_with_emails)} skipped_large={self._skipped_too_large} skipped_recruiter={self._skipped_has_recruiter} total_time={total_time:.1f}s")

        # Save CSV
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        csv_path = f".tmp/reed_jobs_{timestamp}.csv"
        ...
        if jobs_with_emails:
            ...
            # Export to Sheets
            sheet_title = f"Reed Jobs - {keyword} - {datetime.now().strftime('%Y-%m-%d')}"
    ...
    def execute_batch(self, keywords: List[str], location: str = "London", posted_date: str = "last14days",
                      max_jobs_per_keyword: int = 50, max_pages: int = 20,
                      min_age: int = 0, max_size: int = 300, skip_recruiter_check: bool = False, skip_size_check: bool = False):
        """
        Batch execution: run multiple keywords, deduplicate companies across all,
        enrich once, export to single Google Sheet.
        """
        start_time = time.time()
        total_target = max_jobs_per_keyword * len(keywords)
        ...
        print("\n" + "="*70)
        print("🚀 REED.CO.UK BATCH JOB SCRAPER & OUTREACH SYSTEM")
        print(f"Keywords: {len(keywords)} | Jobs/keyword: {max_jobs_per_keyword} | Target: ~{total_target}")
        print(f"Location: {location} | Posted: {posted_date}")
        print(f"Signals: Size≤{max_size} | Recruiter={'OFF' if skip_recruiter_check else 'ON'} | MinAge={min_age}d")
        print("="*70)

        # ── Phase 1: Start ALL Apify runs ──
        print(f"\n📡 Phase 1: Starting {len(keywords)} scrapers...")
        run_ids = {}
        for i, kw in enumerate(keywords, 1):
            run_id = self.start_scraping_job(kw, location, max_jobs=max_jobs_per_keyword, max_pages=max_pages)
            if run_id:
                run_ids[kw] = run_id
                print(f"   ✓ [{i}/{len(keywords)}] Started: '{kw}'")
            else:
                print(f"   ✗ [{i}/{len(keywords)}] Failed: '{kw}'")
            time.sleep(0.5)

        if not run_ids:
            print("❌ All scrapers failed to start")
            notify_error()
            return

        # ── Phase 2: Stream & aggregate all jobs (deduplicate by company) ──
        print(f"\n📥 Phase 2: Streaming jobs from {len(run_ids)} runs...")
        all_jobs = []
        seen_companies = set()

        for kw, run_id in run_ids.items():
            keyword_jobs = 0
            keyword_dupes = 0
            for job in self.stream_jobs(run_id, max_jobs=max_jobs_per_keyword):
                company = job['company_name'].lower().strip()
            ...
            dupe_note = f" ({keyword_dupes} dupes skipped)" if keyword_dupes else ""
            print(f"   ✓ '{kw}': {keyword_jobs} unique companies{dupe_note}")

        print(f"\n   📊 Total unique companies: {len(all_jobs)}")

        if not all_jobs:
            print("⚠️ No jobs found across all keywords")
            notify_error()
            return

        # ── Phase 3: Enrich all companies in parallel ──
        print(f"\n🔍 Phase 3: Enriching {len(all_jobs)} companies (workers={self.MAX_WORKERS})...")
        processed_jobs = []

        with ThreadPoolExecutor(max_workers=self.MAX_WORKERS) as executor:
            futures = {
                executor.submit(self.process_single_company, job): job
                for job in all_jobs
            }
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    if results:
                        ...
                        dm_count = len(results)
                        print(f"   ✓ [{completed}/{total}] {results[0]['company_name']} ({dm_count} DMs)")
                    else:
        ...
        jobs_with_emails = [j for j in processed_jobs if j.get('dm_email') and j.get('email_status') == 'found']
        num_companies = len(seen_companies)

        total_time = time.time() - start_time
        avg_time = total_time / num_companies if num_companies else 0
        ...
        print(f"   Keywords Searched: {len(run_ids)}")
        print(f"   Companies Processed: {num_companies}")
        print(f"   Skipped (>{self._max_company_size} employees): {self._skipped_too_large}")
        print(f"   Skipped (Internal Recruiter): {self._skipped_has_recruiter}")
        print(f"   High Pain (30+ days): {high_pain}")
        ...
        print(f"   Avg Time/Company: {avg_time:.1f}s")
        print(f"   Cache Hits - Websites: {len(self._website_cache)} | Decision Makers: {len(self._dm_cache)}")
        print("-"*70 + "\n")

        logger.info(f"BATCH METRICS: keywords={len(run_ids)} companies={num_companies} dms={len(jobs_with_emails)} skipped_large={self._skipped_too_large} skipped_recruiter={self._skipped_has_recruiter} total_time={total_time:.1f}s")

        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        csv_path = f".tmp/reed_jobs_batch_{timestamp}.csv"

        if jobs_with_emails:
            pd.DataFrame(jobs_with_emails).to_csv(csv_path, index=False)
            print(f"📁 CSV Backup: {csv_path}")

            sheet_title = f"Reed Jobs - Account Mgmt & CS Batch - {datetime.now().strftime('%Y-%m-%d')}"
            url = self.export_to_google_sheets(jobs_with_emails, sheet_title)
            if url:
                print(f"🔗 Google Sheet: {url}")
        else:
...
def main():
    ...
    import argparse
    parser = argparse.ArgumentParser(description='Reed.co.uk Job Scraper & Outreach System')

    parser.add_argument('--keyword', type=str, default=None, help='Single job search keyword')
    parser.add_argument('--keywords', type=str, default=None,
                        help='Multiple keywords separated by | (pipe). E.g. "Account Manager|Customer Success"')
    parser.add_argument('--location', type=str, default="London", help='Job location (default: London)')
    parser.add_argument('--posted-date', type=str, default="last14days",
                        choices=['last24hours', 'last3days', 'last7days', 'last14days', 'lastMonth'],
                        help='Date filter')
    parser.add_argument('--limit', type=int, default=50, help='Max jobs per keyword (default: 50)')
    parser.add_argument('--max-pages', type=int, default=20, help='Max pages to scrape')
    parser.add_argument('--min-age', type=int, default=0, help='Min job age in days (30 = pain signal)')
    parser.add_argument('--max-size', type=int, default=300, help='Max company size (default: 300)')
    ...
    try:
        scraper = ReedJobScraper()

        if args.keywords:
            # Batch mode: multiple keywords separated by |
            keyword_list = [kw.strip() for kw in args.keywords.split('|') if kw.strip()]
            print(f"📋 Batch mode: {len(keyword_list)} keywords")
            scraper.execute_batch(
                keywords=keyword_list,
                location=args.location,
                posted_date=args.posted_date,
                max_jobs_per_keyword=args.limit,
            ...
            print("❌ Provide --keyword or --keywords")
            sys.exit(1)
