...
class GlassdoorJobScraper:
        ...
        while True:
            ...
            if items:
                for item in items:
                    ...
                    posted_date_raw = item.get('posted_date', '')
    ...
    def is_decision_maker_by_size(self, job_title: str, company_size: str = "medium") -> bool:
        ...
        if not job_title or len(job_title) < 3:
            return False
        jt_lower = job_title.lower()
        exclude_pattern = r'\b(?:assistant|associate|junior|intern|coordinator|analyst|specialist|representative|agent|clerk|trainee|apprentice|student)\b'
        if re.search(exclude_pattern, jt_lower):
            return False
        if company_size == "small":
            dm_pattern = r'\b(?:founder|co-founder|ceo|chief executive|owner|president|managing director)\b'
        elif company_size == "medium":
            dm_pattern = r'\b(?:founder|co-founder|ceo|chief executive|owner|president|managing director|hr manager|human resources manager|people lead|head of people|head of hr|head of human resources|people manager|talent manager|hr lead)\b'
        else:
            dm_pattern = r'\b(?:founder|co-founder|ceo|chief executive|owner|president|managing director|hr director|director of hr|director of human resources|head of hr|head of people|head of human resources|vp of people|vp people|vp hr|vp human resources|chief people officer|cpo)\b'
        return bool(re.search(dm_pattern, jt_lower))
...
"""
    ...
    def execute(self, query: str, location: str = "", country: str = "Canada", max_jobs: int = 10, days_posted: int = 60,
        ...
        print("\n" + "="*70)
        print("🚀 GLASSDOOR JOB SCRAPER & OUTREACH SYSTEM (3-SIGNAL MODE)")
