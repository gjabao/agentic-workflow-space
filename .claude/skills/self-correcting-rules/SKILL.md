---
name: self-correcting-rules
description: >
  Self-correcting rules engine that learns from mistakes. When the agent makes an error or the user corrects it, the agent appends a new rule to this file so the same mistake never recurs. Rules accumulate over time, making the agent increasingly reliable for this project.
